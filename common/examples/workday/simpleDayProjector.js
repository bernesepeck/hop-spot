...
const projectDay = (dayController) => {
  const [amStartViewLabel, amStartViewInput] = projectChangeInput(
    'DAY',
    dayController.amStartCtrl
  );
  const [amEndViewLabel, amEndViewInput] = projectChangeInput(
    'DAY',
    dayController.amEndCtrl
  );
  const [pmStartViewLabel, pmStartViewInput] = projectChangeInput(
    'DAY',
    dayController.pmStartCtrl
  );
  const [pmEndViewLabel, pmEndViewInput] = projectChangeInput(
    'DAY',
    dayController.pmEndCtrl
  );
  ...
  amDiv
    .querySelector('#am_start')
    .replaceWith(amStartViewLabel, amStartViewInput);
  amDiv.querySelector('#am_end').replaceWith(amEndViewLabel, amEndViewInput);
  pmDiv
    .querySelector('#pm_start')
    .replaceWith(pmStartViewLabel, pmStartViewInput);
