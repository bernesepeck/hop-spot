...
const projectWeek = (weekController) => {
  ...
  const [x1, x2, x3, totalElement] = dom(`
    ...
    `);
  // data binding
  weekController.onTotalWeekMinutesChanged(
    (minutes) =>
      (totalElement.firstElementChild.textContent =
        totalMinutesToTimeString(minutes))
  );
...
const projectDayWithTotal = (dayController, weekDay) => {
  ...
  /** @type HTMLDivElement */ const weekDayElement = dom(
    `<div>${weekDay}</div>`
  )[0];
  /** @type HTMLDivElement */ const totalElement = dom(
    `<div><output>00:00</output></div>`
  )[0];
  ...
  [amDiv, pmDiv]
    .flatMap((el) => Array.from(el.querySelectorAll('label')))
    .forEach(
      (label) => (label.textContent = weekDay + ' ' + label.textContent)
    );

  dayController.onTotalChanged(
    (minutes) =>
      (totalElement.firstElementChild.textContent =
        totalMinutesToTimeString(minutes))
  );
